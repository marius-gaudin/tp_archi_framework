...
const checkConfig = () => {
  ...
  try {
      ...
      for (const step in config.steps ) {
        if(step && step!="") {
          ...
          if(config.steps[step].hasOwnProperty("next")) {
            if(config.steps != step && !config.steps.hasOwnProperty(config.steps[step].next))
    ...
    if(e.code == "ENOENT") {
      console.error("Le fichier config-filters.json n'existe pas ! il faut obligatoirement le définir");
    } else {
      console.error("Un problème est survenu dans le fichier config-filters.json : ");
      console.error(e);
    }
    return false;
  }
